...
\toprule
\textbf{Class} & \textbf{Size $|C_i|$} & \textbf{Order in $I^*$} & \textbf{Order in $A_5$} & \textbf{Real Part $a_i$} & \textbf{Rotation Angle $\theta_i = \arccos a_i$} \\
\midrule
...
\end{equation}
\item \textbf{Spatial Heat Kernel Correction}: The coefficient $a_4(S^3/I^*)$ provides the exact leading positive curvature-squared correction $\int_{S^3/I^*} (5\mathcal{R}^2 - 2|\Ric|^2 + 2|\Riem|^2) d\mathrm{vol}$ to the 3D spatial Laplace--Beltrami heat kernel.
\end{enumerate}
\end{theorem}
...
\end{equation}
where $Y_2 = \Tr(Y_u^\dagger Y_u + Y_d^\dagger Y_d + Y_\nu^\dagger Y_\nu + Y_e^\dagger Y_e)$ and $Y_4 = \Tr((Y_u^\dagger Y_u)^2 + \dots)$.
\item \textbf{Electroweak Boson and Higgs Mass Relations}:
\begin{equation}
...
All core algebraic structures, group representation dimensions, Seeley--DeWitt heat kernel asymptotic relations (including the fourth coefficient $a_4$), and Noncommutative Standard Model spectral triples presented in this monograph have been formally formalized and machine-checked in Lean 4 (toolchain \texttt{v4.34.0-rc2} with Mathlib). The formalization resides in the \texttt{Formalization.PoincareDodecahedron} module tree within the public repository at \url{https://github.com/sneed-and-feed/original-research} (commit \texttt{cc2317b}), compiling with 0 errors, 0 warnings, and 0 custom axioms beyond Lean's standard kernel (\texttt{propext}, \texttt{Quot.sound}, \texttt{Classical.choice}). Table \ref{tab:lean} maps each analytical result to its verified formal declaration.
...
$\SU(2)$ Quaternionic Character Formula $\chi_\ell(u)$ & \texttt{SpectralDecomposition.lean} & \texttt{chi\_SU2}, \texttt{chi} \\
...
Discrete Heat Kernel Trace $Z(t)$ on $S^3/I^*$ & \texttt{SpectralDecomposition.lean} & \texttt{heatTrace} \\
...
Higgs Potential Minimum at $v^2$ & \texttt{StandardModel.lean} & \texttt{higgs\_potential\_minimum} \\
