...
The following medium-term research milestones (3–9 months) outline targeted formalization pathways to deepen the arithmetic, geometric, and topological results of the repository:

### 1. Full Moduli & Degeneration Story
* **Schmid's Nilpotent Orbit Theorem**: Deepen the formalization of Schmid's period map asymptotics $\tau(t) = \exp(t N) \cdot \tau_{\text{nilp}} + \mathcal{O}(e^{-c \operatorname{Im} t})$ on the Siegel upper half-space $\mathbb{H}_2$.
...
* **Orbifold Gauss–Bonnet Interaction**: Couple the Gauss–Bonnet hyperbolic area formula $\operatorname{Area}(\mathcal{O}) = 2\pi(1 - 1/p - 1/q)$ directly with the symplectic volume of moduli fibers.
* **Eisenstein Scattering Determinants**: Formally verify the spectral-geometric residue identity $\operatorname{Res}_{s=1} \phi(s) \cdot \operatorname{Area}(\mathcal{O}) = 2\pi$ across continuous infinite families of hyperbolic 2-orbifolds.
