"""
Compilation & Verification Pipeline for Poincaré Dodecahedral Adèlic Cosmology Monograph
Compiles the complete monograph into publication-grade 'paper/main.pdf'
and runs comprehensive quality verification checks.
"""
...
def compile_pdf():
    print("Building Typst monograph specification...")
    build_typst_monograph()
    
    print("Compiling paper/main.typ -> paper/main.pdf...")
    typst.compile("paper/main.typ", output="paper/main.pdf")
    print("[SUCCESS] paper/main.pdf compiled successfully!")

def verify_pdf():
    print("\n--- Verifying PDF Integrity & Typesetting Quality ---")
    doc = pymupdf.open("paper/main.pdf")
    num_pages = len(doc)
    print(f"Total Pages: {num_pages}")
    assert num_pages >= 3, f"Monograph too short: {num_pages} pages"
    ...
    for page_idx, page in enumerate(doc):
        ...
        print(f"Page {page_idx + 1}: {len(text)} characters, {len(images)} embedded images")
    
    # Check critical content markers
    markers = [
        "Poincaré Dodecahedral Adèlic Cosmology",
        ...
        "Seeley",
        "Early Dark Energy",
        "Concordance Scorecard",
        "Formal Lean 4 Theorem Cross-References",
        "48",
        "96",
    ...
    for marker in markers:
        assert marker.lower() in total_text.lower(), f"Missing marker: {marker}"
        print(f"[PASS] Verified presence of section/marker: '{marker}'")
