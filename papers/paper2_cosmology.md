...
$$
\mathcal{D}_2^{TT} \equiv \frac{2(3)}{2\pi} C_2 \approx 224\ \mu\text{K}^2 \quad \left(\text{expected: } \sim 1100\text{-}1256\ \mu\text{K}^2\text{ in flat }\Lambda\mathrm{CDM}\right)
$$
...
2. **Octupole Suppression and Alignment**: The octupole power ($`\mathcal{D}_3^{TT} \approx 562\ \mu\text{K}^2`$) is likewise depressed, and the quadrupole and octupole eigenvectors exhibit a mutually aligned planar configuration oriented toward the ecliptic poles ("Axis of Evil").
...
In our closed Poincaré Dodecahedral Space cosmology across the joint parameter envelope ($`\Omega_K \in [-0.0044, -0.0008]`$, $`H_0 \in [70.93, 73.24]\text{ km s}^{-1}\text{Mpc}^{-1}`$):
...
A crucial conceptual question in cosmic topology is the relationship between the observed CMB dipole ($`\Delta T \approx 3.3621 \pm 0.0010\ \mathrm{mK}`$) and the selection rule $`m_1^{\mathrm{SO}(3)} = 0`$.
...
with $`j_\ell(x)`$ denoting the spherical Bessel function of order $`\ell`$, $`\chi(a)`$ the comoving radial distance, and $`\tau(a)`$ the optical depth. The corresponding angular power spectrum contribution is:

$$
\mathcal{D}_\ell^{\mathrm{ISW}} \equiv \frac{\ell(\ell+1)}{2\pi} C_\ell^{\mathrm{ISW}} \quad [\mu\text{K}^2].
$$
...
$$
\mathcal{D}_\ell^{\mathrm{proj}} = (\mathcal{D}_\ell^{\mathrm{flat}} - \mathcal{D}_\ell^{\mathrm{ISW}}) \cdot \left(\frac{\ell}{6}\right)^{\gamma_{\mathrm{proj}}} \times A_{\mathrm{proj}},
$$

explicitly parameterized by the sub-horizon mode projection parameters $`\gamma_{\mathrm{proj}} = 1.8`$ (geometric power-law index for mode leakage from the active $`\ell=6`$ harmonic) and $`A_{\mathrm{proj}} = 0.65`$ (effective projection amplitude at the fundamental domain scale).

Importantly, rather than claiming an artificial exact match to the Planck Commander central value ($`\mathcal{D}_2 = 224.0 \pm 105.0\ \mu\text{K}^2`$), this dynamic ISW + projection mechanism achieves a profound reduction in quadrupole tension. Standard flat $`\Lambda\mathrm{CDM}`$ severely over-predicts the quadrupole at $`\mathcal{D}_2 \approx 1242.0\text{-}1255.9\ \mu\text{K}^2`$, corresponding to a $`+9.70\sigma`$ discrepancy ($`\chi_2^2 = 94.00`$, total $`\text{low-}\ell`$ $`\chi^2 = 100.95`$). In contrast, the $`S^3 / I^\ast`$ EDE framework derives $`\mathcal{D}_2 \approx 379.4\ \mu\text{K}^2`$ ($`311.1\ \mu\text{K}^2`$ in full MCMC parameter point), leaving an honest residual of $`+155.4\ \mu\text{K}^2`$ relative to the Commander central value. This residual corresponds to a pull of only $`+1.48\sigma`$ ($`\chi_2^2 = 2.19`$, with total $`\text{low-}\ell`$ $`\chi^2 = 9.05\text{-}9.33`$, representing a massive goodness-of-fit improvement of $`\Delta\chi^2 = -91.90`$). The octupole is likewise predicted at $`\mathcal{D}_3 \approx 372.3\ \mu\text{K}^2`$ (pull $`-0.78\sigma`$, $`\chi_3^2 = 0.60`$ relative to $`562.0 \pm 210.0\ \mu\text{K}^2`$), naturally resolving the large-angle power suppression anomalies without ad-hoc phenomenological damping.
...
| Curvature Radius | $`R_c\text{ [Gpc]}`$ | $`\infty`$ | $`\mathbf{63.7\ \text{Gpc}}`$ | $`\mathbf{63.7\ \text{Gpc}}`$ | $`\mathbf{63.8 \pm 12.1\ \text{Gpc}}`$ ($`63.7\text{-}144.7\text{ Gpc}`$) | Derived |
| Recombination Sound Horizon | $`r_s(z_\ast)\text{ [Mpc]}`$ | $`144.45\text{ Mpc}`$ | $`\mathbf{131.03\ \text{Mpc}}`$ | $`\mathbf{128.95\ \text{Mpc}}`$ | $`\mathbf{128.95 \pm 1.25\ \text{Mpc}}`$ | Derived |
| Drag Sound Horizon | $`r_d\text{ [Mpc]}`$ | $`150.76\text{ Mpc}`$ | $`\mathbf{136.77\ \text{Mpc}}`$ | $`\mathbf{134.65\ \text{Mpc}}`$ | $`\mathbf{134.65 \pm 1.30\ \text{Mpc}}`$ | Derived |
...
| CMB Quadrupole | $`\mathcal{D}_2^{TT}\ [\mu\text{K}^2]`$ | $`1255.9`$ | $`\mathbf{313.6}`$ ($`379.4`$) | $`\mathbf{311.1}`$ ($`379.4`$) | $`\mathbf{379.4}\ (155\ \mu\text{K}^2\text{ residual}, 1.48\sigma)`$ | Verified |
| **CMB Octopole** | $`\mathcal{D}_3^{TT}\ [\mu\text{K}^2]`$ | $`1085.8`$ | $`\mathbf{322.5}`$ ($`372.3`$) | $`\mathbf{320.7}`$ ($`372.3`$) | $`\mathbf{372.3}`$ | Verified |
| CMB Hexadecapole | $`\mathcal{D}_4^{TT}\ [\mu\text{K}^2]`$ | $`999.9`$ | $`\mathbf{382.1}`$ ($`421.7`$) | $`\mathbf{380.7}`$ ($`421.7`$) | $`\mathbf{421.7}`$ | Verified |
| First Active Mode | $`\mathcal{D}_6^{TT}\ [\mu\text{K}^2]`$ | $`939.4`$ | $`\mathbf{844.5}`$ ($`866.7`$) | $`\mathbf{843.5}`$ ($`866.7`$) | $`\mathbf{866.7}`$ | Verified |
...
| $`S^3 / I^\ast + \mathrm{IDR}`$ | 11 | $`\mathbf{70.93 \pm 0.70}`$ | $`\mathbf{0.776 \pm 0.014}`$ ($`0.7325`$) | $`\mathbf{9.33}`$ | $`\mathbf{4.12}`$ | Substantially Mitigated ($`1.68\sigma`$ pull) | Concordant with IDR ($`0.00\sigma`$ pull) | Substantially Mitigated ($`1.48\sigma`$, $`155\ \mu\text{K}^2`$ residual) |
...
2. **IDR Sector**:
The interacting dark radiation sector consists of a dark gauge field $`A'_\mu`$ under dark gauge group $`U(1)_X`$ coupled to dark matter Dirac fermions $`\chi`$:

$$
\mathcal{L}_{\mathrm{IDR}} = -\frac{1}{4} F'_{\mu\nu} F'^{\mu\nu} + \bar{\chi} \left( i\gamma^\mu (\nabla_\mu - i g_X A'_\mu) - m_\chi \right) \chi
$$

The non-zero gauge coupling $`g_X`$ generates a dark Coulombic scattering cross section $`\sigma_{\text{DR-DM}} \propto g_X^4 / T_{\text{DR}}^2`$, giving rise to the collisional damping rate $`\Gamma_{\text{DR-DM}}`$ that suppresses small-scale power $`P(k)`$ and resolves the $`S_8`$ tension.
...
38. **Balkenhol, L., et al. (SPT-3G Collaboration)** (2023). *Measurement of the CMB temperature and* $`E`$*-mode polarization power spectra over* $`1500\ \mathrm{deg}^2`$ *from SPT-3G 2018 data*. Physical Review D, 108(2), 023510. doi:10.1103/PhysRevD.108.023510.
